...
function ProjectRow({ project: p, isSelected, descWidth }: ProjectRowProps) {
...
export function ProjectListView({
  ...
  const archivedLines = archived.length > 0 ? 2 : 0
  const maxVisible = Math.max(4, termRows - CHROME_LINES - archivedLines)
  ...
  useEffect(() => {
    ...
    else if (sel >= offset + visible) setOffset(Math.max(0, sel - visible + 1))
  }, [sel, offset, visible])
  ...
  useInput((input, key) => {
    if (key.upArrow || input === "k") {
      ...
      if (next.length === 0 && archived.length === 0) exit()
  ...
  if (items.length === 0) {
    return (
      <Box flexDirection="column">
        ...
        {archived.length > 0 && (
          <Text color={theme.muted}>Archived (no longer in DB): {archived.join(", ")}</Text>
        )}
      </Box>
    )
  }
